...
//TODO: delete a user account - done
//TODO: update a user account - done

//TODO: create a scream - done - working
//TODO: update a scream -
...
//post a new scream
router.post('/', [isAuth, [check('body').notEmpty()]], async (req, res) => {
...
router.post('/:screamId/like', isAuth, async (req, res) => {
	...
	try {
		...
		let foundUserLike = scream.likes.some(function(like) {
		...
		if (foundUserLike) {
			...
			scream.likes.pull(req.user.id);
			message = 'unliked successfully';
		} else {
			// adding the new user like
			scream.likes.push(req.user.id);
			message = 'liked successfully';
		}

		await scream.save();
		return res.status(200).json({ message, scream });
...
// OLD CODE
// router.put('/:screamId/unlike', isAuth, async (req, res) => {
// 	try {
...
router.delete('/:screamId/', isAuth, async (req, res) => {
...
router.post('/:screamId/:commentId/like', isAuth, async (req, res) => {
	...
	try {
		...
		let foundUserLike = comment.likes.some(function(like) {
			return like.equals(req.user.id);
		});
...
// OLD CODE
// router.put('/:screamId/:commentId/unlike', isAuth, async (req, res) => {
// 	try {
// 		//we have the scream id
// 		// we have the user
// 		const comment = await Comment.findById(req.params.commentId);
...
router.delete('/:screamId/:commentId/', isAuth, async (req, res) => {
	//delete the comment
	try {
